...
autocmd BufNewFile,BufReadPre *.js let b:syntastic_javascript_eslint_args =
    \ '--config ' . findfile('.eslintrc', escape(expand('%:p:h'), ' ') . ';')

autocmd BufNewFile,BufReadPre *.jsx let b:syntastic_javascript_eslint_args =
    \ '--config ' . findfile('.eslintrc', escape(expand('%:p:h'), ' ') . ';')
